...
class PrefManager(var context: Context) {
    ...
    fun clearPreference(){
        ...
        (context as dsc.app.reckon.BaseActivity).finish()
    }
    ...
    fun writeSelectedAgeRange(selectedAgeRange: AgeRange){
        val editor = getLiveStockPrefs().edit()
        val gson = Gson()
        val json = gson.toJson(selectedAgeRange)
        editor.putString(AGE_RANGE_KEY, json)
        editor.apply()
    }

    fun getSelectedAgeRange() : AgeRange{
        val gson = Gson()
        val json = getLiveStockPrefs().getString(AGE_RANGE_KEY, "")
        return gson.fromJson(json, AgeRange::class.java)
    }
